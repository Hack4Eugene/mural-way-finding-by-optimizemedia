...
class w2dc_listings_manager {
	...
	public function __construct() {
		...
		if ((isset($_POST['publish']) || isset($_POST['save']) || isset($_POST['w2dc_save_as_active'])) && (isset($_POST['post_type']) && $_POST['post_type'] == W2DC_POST_TYPE)) {
	...
	public function listingExpirationDateMetabox($post) {
		...
		if ($listing->status != 'expired') {
			...
			if (get_option('w2dc_fsubmit_addon') && isset($w2dc_instance->dashboard_page_url) && $w2dc_instance->dashboard_page_url)
				echo '<br /><a href="' . w2dc_dashboardUrl(array('w2dc_action' => 'renew_listing', 'listing_id' => $listing->post->ID)) . '"><img src="' . W2DC_RESOURCES_URL . 'images/page_refresh.png" class="w2dc-field-icon" />' . $renew_link . '</a>';
			else
		...
		if (isset($_GET['listings_ids'])) {
			...
			if (isset($_GET['upgrade_action']) && $_GET['upgrade_action'] == 'upgrade') {
				...
				if ($w2dc_form_validation->run()) {
					...
					if ($upgraded)
						w2dc_addMessage(sprintf(_n('%d listing have changed level successfully!', '%d listings have changed levels successfully!', $upgraded, 'W2DC'), $upgraded));
				} else
					exit();
			}
	...
	public function saveInitialDraft($post_id) {
		...
		if ($w2dc_instance->current_directory) {
			add_post_meta($post_id, '_directory_id', $w2dc_instance->current_directory->id);
		}
	...
	public function saveDirectoryMeta($maybe_empty, $postarr) {
		...
		if (isset($_POST['directory_id']) && is_numeric($_POST['directory_id']) && ($directory = $w2dc_instance->directories->getDirectoryById($_POST['directory_id']))) {
			if ($this->current_listing->post->ID != $directory->id) {
